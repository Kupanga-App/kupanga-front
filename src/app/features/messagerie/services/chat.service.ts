  ...
  private normalizeConversationPage(raw: Record<string, unknown>): ConversationPageDTO {
    ...
    return {
      contenu: contenu.map((c) => ({
        ...
        expediteurName: ((c['expediteurName'] ?? '') as string),
        destinataireName: ((c['destinataireName'] ?? '') as string),
        expediteurPhotoUrl: ((c['expediteurPhotoUrl'] ?? '') as string),
        destinatairePhotoUrl: ((c['destinatairePhotoUrl'] ?? '') as string),
